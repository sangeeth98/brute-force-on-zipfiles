...
def brute_force_attack(zip_file):
    ...
    for n in range (6,7):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            results = [executor.submit(processthread, it, zip_file) for _ in range(200)]
            if(True in results): return True
            if(2 in results): done_looping = True
    #returns false if not succesful#
    return False

def processthread(it, zip_file):
    stop = False
    while(not stop):
        try:
            letters = next(it)
        except StopIteration: stop = True
